...
void Status::mountScan() {
  // Probe immediately when unknown, and retry a failed detection every 10s.
  // This avoids permanently hiding the Mount page link when a startup-time
  // probe fails during an OnStep restart/power glitch window.
  unsigned long t = millis();
  if (mountFound == SD_UNKNOWN || (mountFound == SD_FALSE && (t - lastMountScanTime) > 10000)) {
    char result[80] = "";
    if (!onStep.command(":GU#", result) || result[0] == 0) {
      if (mountFound == SD_UNKNOWN) mountFound = SD_FALSE;
    } else mountFound = SD_TRUE;
    lastMountScanTime = t;
    delay(0);
  }
}
