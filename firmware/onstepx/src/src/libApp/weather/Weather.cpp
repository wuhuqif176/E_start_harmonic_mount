...
bool Weather::init() {
    ...
    #elif WEATHER == AHT20
      #if defined(I2C_SDA_PIN) && defined(I2C_SCL_PIN) && I2C_SDA_PIN != OFF && I2C_SCL_PIN != OFF
        HAL_WIRE.begin(I2C_SDA_PIN, I2C_SCL_PIN);
      #endif
      if (aht.begin(&HAL_WIRE, 0, 0x38)) {
        weatherSensor = WS_AHT20; success = true;
        DLF("MSG: Weather.init(), AHT20 (I2C 0x38) initialized successfully");
      } else { DLF("WRN: Weather.init(), AHT20 (I2C 0x38) not found"); }
    #else
    ...
    #if WEATHER == BME280 || WEATHER == BME280_0x76 || WEATHER == BMP280_0x76 || WEATHER == BMP280 || WEATHER == AHT20
...
void Weather::poll() {
    ...
    if (success && !xBusy) {
      #if WEATHER == AHT20
        // AHT20: read temperature and humidity in one call
        sensors_event_t humidity_event, temp_event;
        aht.getEvent(&humidity_event, &temp_event);
        temperature = temp_event.temperature;
        humidity = humidity_event.relative_humidity;
        pressure = NAN;  // AHT20 has no pressure sensor
