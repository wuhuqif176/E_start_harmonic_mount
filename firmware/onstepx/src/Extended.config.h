...
#define NV_WIPE                       OFF //        关闭，将默认值写回到非易失性存储器（FLASH、EEPROM 等）  Infreq
...
// ---------------------------------------------------------------------------------------------------------------------------------
// MOTOR DRIVER DETECT --------------------------------------------------------------------------------------------------------------
// Enables startup detection of the stepper motor drivers (TMC UART).
// At boot the firmware verifies two-way UART comms with each TMC2209 and
// prints "motor driver device detected" on success or
// "no motor driver device detected!" on failure (axis stays disabled).
// Remove this define once driver comms have been verified.
#define MOTOR_DRIVER_DETECT